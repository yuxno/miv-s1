...
if image1 is None or image2 is None:
    ...
    residual_map = np.zeros_like(image1, dtype=np.float32)

    # Set a threshold for detecting changes
    mse_threshold = 100  # Adjust as needed
    ...
    residual_map_colored = cv2.applyColorMap((residual_map * 255).astype(np.uint8), cv2.COLORMAP_JET)
    ...
    for i, j, block in blocks_image1:
        ...
        if residual_map[i, j] > mse_threshold:
    ...
    cv2.imshow('MSE Residual Map', residual_map_colored)
